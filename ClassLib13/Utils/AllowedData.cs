...
namespace ClassLib13.Utils
{
    ...
    {        
        ...
        {
            return input == "Tables" ? new List<string>() { "EXCERCISES", "SUBSCRIPTIONS", "CHALLENGES", "ALL" } : new List<string>() { "EXCERCISE", "SUBSCRIPTION", "CHALLENGE", "ALL" };
        }

        public static bool TypeExists(string typeName)
		{
            Console.WriteLine(AllowedTypes.Contains(typeName.ToUpper()));
            return AllowedTypes.Contains(typeName.ToUpper());
        }

        public static bool TableExists(string tableName)
        {
            return AllowedTables.Contains(tableName.ToUpper());
        }

        private static readonly Dictionary<string, List<string>> TypeFilters = new Dictionary<string, List<string>>()
        {
            { "Exercise", new List<string>{ "Sport", "Grade" } },
        ...
        public static string ValidateData(string productType, ref Dictionary<string, string> productFilters)
        {
            ...
            if (!AllowedData.TableExists(productType)) 
            { return $"This type '{productType}' does not exist"; }

            var wrongFiltersArray = productFilters.Keys.Where(key => !GetFiltersForTable(productType).Contains(key)).ToList();
            if (wrongFiltersArray.Count > 0) 
            { return $"Error: ({string.Join(", ", wrongFiltersArray)}) these filters don't exist for '{productType}'... I cancelled the operation"; }
        ...
        {
            if (!values.Any())
            { return $"No update values found ({values})"; }
